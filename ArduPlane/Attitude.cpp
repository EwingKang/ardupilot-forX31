...
void Plane::fly_by_wire_ewing(float speed_scaler)
{
    ...
    if ((aero_available)&&(vel_available)) {
        ...
        channel_roll->set_servo_out(svo_aileron);
        channel_pitch->set_servo_out(constrain_float(svo_canard, -4500, 4500));
        stabilize_yaw(speed_scaler);
    }else {
...
void Plane::stabilize()
{
    ...
    } else if (control_mode == NDI_EW) {    //EWING NDI controller
        ndi_ewing(speed_scaler);
